 ...
 */
function ip_master_categorized_blog() {

	$category_count = get_transient( 'ip_master_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'ip_master_categories', $category_count );
	}
 ...
 */
function ip_master_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function ip_master_get_placeholder_unsplash( $args = array() ) {
	...
	if ( ! is_numeric( $args['width'] ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'ip_master' ) );
	}

	if ( ! is_numeric( $args['height'] ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'ip_master' ) );
	}
	...
	$alt = apply_filters( 'ip_master_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'ip_master' ) );
 ...
 * @return string
 */
function ip_master_copyright_year( $atts ) {
...
add_shortcode( 'ip_master_copyright_year', 'ip_master_copyright_year', 15 );
...
if ( defined( 'WPSEO_VERSION' ) ) {
	 ...
	 * @since  NEXT
	 */
	function ip_master_move_yoast_to_bottom() {
		return 'low';
	}
	add_filter( 'wpseo_metabox_prio', 'ip_master_move_yoast_to_bottom' );
}
 ...
 * @author jomurgel
 */
function ip_master_get_the_content( $content ) {
...
add_filter( 'the_content', 'ip_master_get_the_content', 20 );
