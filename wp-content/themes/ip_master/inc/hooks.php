 ...
 * @return array
 */
function ip_master_body_classes( $classes ) {
...
add_filter( 'body_class', 'ip_master_body_classes' );

/**
 * Flush out the transients used in ip_master_categorized_blog.
 *
 * @author WDS
 * @return string
 */
function ip_master_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}
	// Like, beat it. Dig?
	delete_transient( 'ip_master_categories' );
}
add_action( 'delete_category', 'ip_master_category_transient_flusher' );
add_action( 'save_post', 'ip_master_category_transient_flusher' );
 ...
 * @return string
 */
function ip_master_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'ip_master' ) . '...</a>';
}
add_filter( 'the_content_more_link', 'ip_master_content_more_link' );
 ...
 * @return string
 */
function ip_master_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'ip_master' ) );
}
add_filter( 'excerpt_more', 'ip_master_excerpt_more' );
 ...
 * @return array
 */
function ip_master_custom_mime_types( $mimes ) {
	$mimes['svg']  = 'image/svg+xml';
	$mimes['svgz'] = 'image/svg+xml';
	return $mimes;
}
add_filter( 'upload_mimes', 'ip_master_custom_mime_types' );
...
add_shortcode( 'svg', 'ip_master_display_svg' );
 ...
 * @return string
 */
function ip_master_display_customizer_header_scripts() {

	// Check for header scripts.
	$scripts = get_theme_mod( 'ip_master_header_scripts' );
	...
	echo ip_master_get_the_content( $scripts ); // WPCS XSS OK.
}
add_action( 'wp_head', 'ip_master_display_customizer_header_scripts', 999 );
 ...
 * @return string
 */
function ip_master_display_customizer_footer_scripts() {
...
add_action( 'wp_footer', 'ip_master_display_customizer_footer_scripts', 999 );
 ...
 */
function ip_master_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'ip_master' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'ip_master' ) : esc_html__( 'tagged with', 'ip_master' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = $card_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'ip_master' ), $specify, $term_name );
	}
	...
	if ( is_search() ) {
		...
		$card_long_description = $card_description = sprintf( esc_html__( 'Search results for %s.', 'ip_master' ), $search_term );
	}
...
add_action( 'wp_head', 'ip_master_add_og_tags' );
 ...
 * @author Corey Collins
 */
function ip_master_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'ip_master_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function ip_master_remove_empty_p_tags_from_content( $content ) {
...
add_filter( 'the_content', 'ip_master_remove_empty_p_tags_from_content' );
 ...
 * @author Corey Collins
 */
function ip_master_add_chrome_lazy_loading_to_images( $attr ) {

	$attr['loading'] = 'lazy';

	return $attr;
}
add_filter( 'wp_get_attachment_image_attributes', 'ip_master_add_chrome_lazy_loading_to_images' );
 ...
 * @author Corey Collins
 */
function ip_master_allow_img_attributes( $allowedposttags ) {
...
add_filter( 'wp_kses_allowed_html', 'ip_master_allow_img_attributes', 1 );
 ...
 * @author Corey Collins
 */
function ip_master_add_lazy_loading_to_image_block( $block_content, $block ) {
...
add_filter( 'render_block', 'ip_master_add_lazy_loading_to_image_block', 10, 2 );
