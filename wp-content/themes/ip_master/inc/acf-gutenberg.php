...
function ip_master_acf_init() {
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-accordion',
			'title'           => esc_html__( 'Accordion', 'ip_master' ),
			'description'     => esc_html__( 'A custom set of collapsable accordion items.', 'ip_master' ),
			'render_callback' => 'ip_master_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'sort',
			'keywords'        => array( 'accordion', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'ip_master_acf_enqueue_accordion_scripts',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'title'           => esc_html__( 'Accordion Block Title', 'ip_master' ),
						'text'            => esc_html__( 'Accordion Block Content', 'ip_master' ),
						'accordion_items' => array(
							'0' => array(
								'accordion_title' => esc_html__( 'Accordion Item Title', 'ip_master' ),
								'accordion_text'  => esc_html__( 'Accordion Item Content', 'ip_master' ),
							),
						),
					),
				),
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-cta',
			'title'           => esc_html__( 'Call To Action', 'ip_master' ),
			'description'     => esc_html__( 'A call to action block.', 'ip_master' ),
			'render_callback' => 'ip_master_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'megaphone',
			'keywords'        => array( 'call to action', 'cta', 'wds' ),
			'mode'            => 'preview',
			'enqueue_assets'  => 'ip_master_acf_enqueue_backend_block_styles',
			'align'           => 'wide',
			'supports'        => $supports,
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'title'       => esc_html__( 'Call To Action Title', 'ip_master' ),
						'text'        => esc_html__( 'Call To Action Text', 'ip_master' ),
						'button_link' => array(
							'title' => esc_html__( 'Learn More', 'ip_master' ),
							'url'   => '#',
						),
					),
				),
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Fifty/Fifty Block', 'ip_master' ),
			'description'     => esc_html__( 'A split-content block with text and/or media.', 'ip_master' ),
			'render_callback' => 'ip_master_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'editor-table',
			...
			'example'         => array(
				'attributes' => array(
					'data' => array(
						'text_primary' => esc_html__( 'Primary Text content', 'ip_master' ),
						'media_right'  => array(
	...
	acf_register_block_type(
		array(
			'name'            => 'wds-hero',
			'title'           => esc_html__( 'Hero Block', 'ip_master' ),
			'description'     => esc_html__( 'A hero with an optional call to action.', 'ip_master' ),
			'render_callback' => 'ip_master_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'slides',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Recent Posts Block', 'ip_master' ),
			'description'     => esc_html__( 'A set of recent posts displayed by category and/or tag.', 'ip_master' ),
			'render_callback' => 'ip_master_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
	...
	acf_register_block_type(
		array(
			...
			'title'           => esc_html__( 'Related Posts Block', 'ip_master' ),
			'description'     => esc_html__( 'A set of manually selected posts.', 'ip_master' ),
			'render_callback' => 'ip_master_acf_block_registration_callback',
			'category'        => 'wds-blocks',
			'icon'            => 'admin-page',
...
function ip_master_add_block_categories( $categories, $post ) {
	...
	return array_merge(
		...
		array(
			array(
				'slug'  => 'wds-blocks',
				'title' => esc_html__( 'WDS Blocks', 'ip_master' ),
			),
		)
	);
}
...
function ip_master_acf_enqueue_carousel_scripts() {
	...
	wp_register_script( 'slick-carousel-js', get_template_directory_uri() . '/dist/slick-carousel/slick/slick.min.js', array( 'jquery' ), '1.8.1', true );
